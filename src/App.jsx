  ...
  return (
    <main className='h-screen bg-slate-800 flex justify-center items-center flex-col'>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col min-w-1/6 gap-2 flex flex-col '>
        ...
        <input type="password" className='bg-slate-900 rounded h-10 text-slate-400 pl-2' {...register('password')} />
        { errors.password && <p className='text-xs text-red-500'>{errors.password.message}</p> }

        <button
          className={ 
            loading ? 'h-10 bg-emerald-600 rounded font-semibold text-slate-300 animate-pulse flex justify-center items-center' : 
      ...
      <p className='text-slate-300 mt-5'>Don't have an account yet? <Link to='/register' className='ml-2 text-cyan-600'>Sign Up</Link></p>
    </main>
  )
}